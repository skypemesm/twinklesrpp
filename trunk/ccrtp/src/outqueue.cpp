...
//std::cout << "Saswat:: PutData in outqueue:Added a new packet to the queue\n\n";


}


void OutgoingDataQueue::setSRPPSockets(int sender_sock,int receiver_sock,int sender_port, int receiver_port,
...
void OutgoingDataQueue::startSRPPsession(int srpp_negotiated_by_sip,sdp_srpp sdpp)
{
	...
	if (srpp::SRPP_Enabled() == 1)
	{
		...
		if (srpp_negotiated_by_sip == 0) // NOT by SIP
		{
